...
// TODO we do not have maybeowned or exclusivelyowned here
#include <atomic>
#include <cassert> // TODO delet
#include <stdexcept>
...
#pragma GCC diagnostic ignored "-Wexceptions"
    ...
    // "Tried to destruct an intrusive_ptr_target that still has intrusive_ptr
    // to it");
    assert(weakcount_.load() == 0);
...
//      intrusive_ptr_target.");
 ...
 public:
  ...
  static intrusive_ptr unsafe_reclaim_from_nonowning(TTarget* raw_ptr) {
    ...
    assert(raw_ptr == NullType::singleton() || raw_ptr->refcount_.load() > 0);
...
class weak_intrusive_ptr final {
 private:
  ...
  void retain_() {
    if (target_ != NullType::singleton()) {
      ...
      assert(new_weakcount != 1);
      //   "weak_intrusive_ptr: Cannot increase weakcount after it reached
      //   zero.");
    }
  }
 ...
 public:
  ...
  static weak_intrusive_ptr reclaim(TTarget* owning_weak_ptr) {
    ...
    // "weak_intrusive_ptr: Can only weak_intrusive_ptr::reclaim() owning
    // pointers that were created using weak_intrusive_ptr::release().");
