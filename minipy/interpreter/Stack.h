...
// element i
static inline Obj& peek(Stack& stack, size_t i, size_t N) {
  return *(stack.end() - N + i);
}
static inline Obj& peek(Stack* stack, size_t i, size_t N) {
  return peek(*stack, i, N);
}
static inline const Obj& peek(const Stack& stack, size_t i, size_t N) {
  return *(stack.end() - N + i);
}
static inline const Obj& peek(const Stack* stack, size_t i, size_t N) {
  return peek(*stack, i, N);
...
// static inline at::ArrayRef<Obj> peekSlice(
//     const Stack& stack,
//     size_t i,
//     size_t len,
//     size_t N) {
//   return at::ArrayRef<Obj>(stack).slice(stack.size() - N + i, len);
// }
// static inline at::ArrayRef<Obj> last(const Stack& stack, size_t N) {
...
static inline Obj pop(Stack& stack) {
...
static inline std::vector<Obj> pop(Stack& stack, size_t n) {
