...
std::optional<SourceRange> SourceView::findSourceRangeThatGenerated(
...
void SourceRange::print_with_context(
  ...
  std::string_view str = source_view_->text();
  if (size() == str.size()) {
  ...
  while (end_line < str.size() && str[end_line] != '\n')
    ++end_line;
  // TODO good assert
  assert(begin_line == 0 || str[begin_line - 1] == '\n');
  assert(end_line == str.size() || str[end_line] == '\n');
  ...
  assert(begin_context == 0 || str[begin_context - 1] == '\n');
  ...
  assert(end_context == str.size() || str[end_context] == '\n');
  ...
  if (highlight) {
    ...
    while (line_start < range_end) {
      ...
      assert(hightlight_begin == 0 || str[hightlight_begin - 1] == '\n');
      assert(highlight_end == range_end || str[highlight_end] == '\n');
