...
struct Mod : public TreeView {
  explicit Mod(const TreeRef& tree) : TreeView(tree) {
    tree->match(TK_MODULE);
  }

  List<Stmt> body() const {
    return List<Stmt>(subtree(0));
  }

  static Mod create(const SourceRange& range, const List<Stmt>& body) {
    return Mod(Compound::create(TK_MODULE, range, {body}));
  }
};
...
struct ClassDef : public Stmt {
  explicit ClassDef(const TreeRef& tree) : Stmt(tree) {
...
struct Const : public Expr {
  ...
  int64_t asIntegral() const {
    try {
      // TODO c10::stoll exists because no stoll in android env
      return std::stoll(subtree(0)->stringValue(), /*pos=*/0, /*base=*/0);
